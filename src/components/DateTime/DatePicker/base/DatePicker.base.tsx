...
import { IDatePickerStyleProps, IDatePickerStyles } from '@fluentui/react/lib/components/DatePicker/DatePicker.types';
import { defaultDatePickerStrings } from '@fluentui/react/lib/components/DatePicker/defaults';
import { ITextField } from '@fluentui/react/lib/components/TextField/TextField';
import { Calendar } from '@fluentui/react/lib/components/Calendar/Calendar';
import { ICalendar } from '@fluentui/react/lib/components/Calendar/Calendar.types';
...
import { FocusTrapZone } from '@fluentui/react/lib/components/FocusTrapZone/FocusTrapZone';
...
import { IDatePickerProps } from '@talxis/react-components/dist/components/DatePicker';

export const processItems = (suffixItems?: ICommandBarItemProps[], disabled?: boolean) => {
  if (!suffixItems) {
      return [];
  }
...
>((propsWithoutDefaults, forwardedRef) => {
  ...
  const getSuffixItems = () => {
      const calendarButtonVisible = !textFieldProps?.readOnly && !props.disabled ? true : false;
      ...
      if(calendarButtonVisible) {
        mergedItems.push({
          ...
          ...props.calendarButtonProps,
          className: `TALXIS__calendar__button ${props.calendarButtonProps?.className ?? ''}`,
          onClick: (ev) => {
              if(props.calendarButtonProps?.onClick) {
                  props.calendarButtonProps.onClick();
              }
              onIconClick(ev as React.MouseEvent<HTMLElement>);
          },
      });
      }
      if(props.suffixItems) {
        mergedItems = [...mergedItems, ...props.suffixItems]
      }
      const items = processItems(mergedItems, props.disabled);
      return items;
  };
