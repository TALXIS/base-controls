/* import { DefaultButton, Dialog, DialogFooter, IDialogProps, PrimaryButton, useTheme } from "@fluentui/react";
...
export const ChangeEditor = (props: IChangeDialogProps) => {
    ...
    const recordChanges = grid.getDataset().getChanges?.() ?? [];
    ...
    const isSaveDisabled = () => {
        ...
        return !grid.getDataset().isValid() || grid.getDataset().hasInvalidChanges();
    }
    ...
    return <Dialog
        ...
        <DialogFooter>
            <PrimaryButton
                ...
                text={activeSaveOperationsCount > 0 ? grid.getLabels()['saving-saving']() : grid.getLabels()['saving-save-all']()}
                onClick={async () => {
                    setActiveSaveOperationsCount(count => count + 1);
                    await Promise.all([...datasetsRef.current.values()].map(dataset => dataset.save?.()));
                    grid.getDataset().clearChanges?.();
                    setActiveSaveOperationsCount(count => count - 1);
                }}
            />
            <DefaultButton
                text={grid.getLabels()['saving-discard-all']()}
                disabled={activeSaveOperationsCount > 0}
                onClick={async (e) => {
                    if (window.confirm(grid.getLabels()['saving-discard-all-confirmation']())) {
                        grid.getDataset().clearChanges?.();
