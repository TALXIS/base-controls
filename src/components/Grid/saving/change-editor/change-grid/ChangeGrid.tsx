...
export const ChangeGrid = (props: IChangeGrid) => {
    ...
    const changedColumns = fieldChangesRef.current.map((change) => {
        return grid.getDataset().columns.find((x) => change.columnName === x.name)!;
    });

    const recordPrimaryName = (() => {
        let result;
        const primaryColumn = grid.getDataset().columns.find((col) => col.isPrimary);
        if (primaryColumn) {
            result = baseRecord.getFormattedValue(primaryColumn.name);
        } else {
            const firstTextColumn = grid.getDataset().columns.find(
        ...
        return result ?? grid.getLabels()["no-name"]();
    })();
    ...
    const getDataset = () => {
            ...
            [
                {
                    ...
                    'valueDesc__virtual': grid.getLabels()["original-value"](),
                },
                {
                    id__virtual: "new",
                    'valueDesc__virtual': grid.getLabels()["new-value"](),
                },
            ], {
        ...
        grid.getDataset().linking.getLinkedEntities().map(x => dataset.linking.addLinkedEntity(x))
                ...
                if (recordId === 'new') {
                    ...
                    record.expressions?.ui.setNotificationsExpression?.(col.name, () => {
                        return [
                            {
                                uniqueId: "clear",
                                title: grid.getLabels()["saving-discard"](),
                                iconName: "EraseTool",
                                compact: true,
                                messages: [],
                                actions: [
