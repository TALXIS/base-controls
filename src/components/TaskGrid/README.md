...
### Production example — Dataverse-backed descriptor

Real-world descriptors delegate views to `TalxisSavedQueryStrategy` and toggle features based on runtime config:

```ts
export class YungoDescriptor implements ITaskGridDescriptor {
    constructor(private _configService: IConfigService) {}

    // onLoadDependencies is awaited before any strategy factory is called.
    public async onLoadDependencies() {
        this._projectRecord = await this._projectDataProvider.getProjectRecord();
    }

    public onCreateSavedQueryStrategy(): ISavedQueryStrategy {
        if (this._configService.getProperty('isTemplateView')) {
            // Template view: single system query, no user views.
            return {
                onGetSystemQueries: async () => [{ id: '...', name: 'All Templates', columns: this._getDefaultColumns(), isFlatListEnabled: false }],
                onEnableUserQueries:    () => false,
                onGetUserQueries:       async () => [],
                onDeleteUserQueries:    async () => { throw new Error('Not applicable'); },
                onUpdateUserQuery:      async () => { throw new Error('Not applicable'); },
                onCreateUserQuery:      async () => { throw new Error('Not applicable'); },
            };
        }
        // Task view: full user-query support backed by talxis_userquery Dataverse records.
        return new TalxisSavedQueryStrategy({
            onGetSystemQueries: async () => [{ id: '...', name: 'All Tasks', columns: this._getDefaultColumns(), isFlatListEnabled: false }],
            recordId: this._projectRecord.getRecordId(),
            ownerId:  this._configService.getProperty('userId'),
        });
    }

    // TalxisCustomColumnsStrategy stores column definitions in talxis_attributedefinition
    // and values in talxis_attributevalue, keyed to the entity + record.
    public onCreateCustomColumnsStrategy() {
        if (this._configService.getProperty('isTemplateView')) return undefined;
        return new TalxisCustomColumnsStrategy({
            entityName: this._projectDataProvider.getEntityName(),
            recordId:   this._projectRecord.getRecordId(),
        });
    }

    public onGetGridParameters(): ITaskGridParameters {
        const isTemplate = this._configService.getProperty('isTemplateView');
        return {
            height: '100%',
            enableRowDragging:              !isTemplate,
            enableEditColumns:              !isTemplate,
            enableHideInactiveTasksToggle:  !isTemplate,
            enableEditColumnsScopeSelector: false,
        };
    }

    // Different entities for task vs template view — native column names differ.
    public onGetNativeColumns(): INativeColumns {
        if (this._configService.getProperty('isTemplateView')) {
            return { subject: 'ntg_name', parentId: 'ntg_parenttasktemplateidid',
                     stackRank: 'ntg_order', path: 'talxis_path', stateCode: 'statecode' };
        }
        return { subject: 'subject', parentId: 'ntg_parenttaskid', stackRank: 'ntg_stackrank',
                 percentComplete: 'percentcomplete', path: 'talxis_path', stateCode: 'statecode' };
    }

    public onGetControlId()        { return this._configService.getControlId(); }
    public onGetAgGridLicenseKey() { return AG_GRID_LICENSE_KEY; }
}
```
...
### Production Dataverse implementation (key patterns)

```ts
export class YungoTaskStrategy implements ITaskDataProviderStrategy {
    constructor(descriptor: IYungoDescriptor, deps: ITaskStrategyDeps) {
    ...
    public async onInitialize(provider: ITaskDataProvider) {
        ...
        // Subscribe to record lifecycle events for expression setup.
        provider.addEventListener('onRecordLoaded',             (record)          => this._onRecordLoaded(record));
        provider.addEventListener('onRecordColumnValueChanged', (record, col, val) => this._onValueChanged(record, col, val));

        this._fetchXmlDataProvider = new FetchXmlDataProvider({
            fetchXml: Liquid.parseAndRenderSync(this._configService.getProperty('fetchXml'), sourceRecord.getRawData()),
            loadAllRecords: true
        });
        ...
        return {
            rawData:  this._fetchXmlDataProvider.getRawData(),
            columns:  await this._enrichColumns(this._fetchXmlDataProvider.getColumns()),
            metadata: this._fetchXmlDataProvider.getMetadata(),
        };
    }

    public async onCreateTask(parentId?: string): Promise<IRawRecord | null> {
        const result = await Xrm.Navigation.navigateTo({
            pageType: 'entityrecord',
            entityName: this._fetchXmlDataProvider.getEntityName(),
            formId: this._configService.getProperty('taskFormId'),
            data: {
                [PROJECT_ID_COL]: this._projectRecord.getRecordId(),
                [PARENT_ID_COL]:  parentId ?? null,
            }
        }, { target: 2, width: { value: 80, unit: '%' } });

        if (!result.savedEntityReference) return null;
        const [rawRecord] = await this.fetchRawRecords([result.savedEntityReference[0].id]);
        // Move new task to the top of its sibling group.
        await this._prependStackRank(rawRecord[PRIMARY_ID], parentId);
        return rawRecord;
    }

    public async onEditTasks(recordIds: string[]): Promise<IEditTasksResult | null> {
        if (recordIds.length === 1) {
            await Xrm.Navigation.navigateTo(
                { pageType: 'entityrecord', entityId: recordIds[0], formId: this._configService.getProperty('taskFormId') },
                { target: 2, width: { value: 80, unit: '%' } }
            );
            const record = this._provider.getRecordsMap()[recordIds[0]];
            return record.isActive() ? { success: true, updatedRecords: await this.fetchRawRecords(recordIds) } : null;
        }
        await Xrm.Navigation.navigateTo(
            { pageType: 'bulkedit', entityName: this._fetchXmlDataProvider.getEntityName(), entityIds: recordIds } as any,
            { target: 2, position: 2 }
        );
        return { success: true, updatedRecords: await this.fetchRawRecords(recordIds) };
    }

    public async onMoveTask(movingId: string, targetId: string, position: 'above' | 'below' | 'child'): Promise<IRawRecord[] | null> {
        const { confirmed } = await this._configService.getPcfContext().navigation.openConfirmDialog({
            title: 'Confirm reorder', text: this._buildMoveMessage(movingId, targetId, position),
        });
        if (!confirmed) return null;
        if (position === 'child') {
            await this._changeParent(movingId, targetId);
            return [];
        }
        await this._changeParent(movingId, this._getParentOf(targetId));
        const { stackRanks } = await this._updateStackRank(movingId, this._getPreviousSibling(targetId, position), targetId);
        return Object.entries(stackRanks ?? {}).map(([id, rank]) => ({
            ...this._provider.getRawDataMap()[id], [STACK_RANK_COL]: rank
        }));
    }

    // Scope the tree to a single task's subtree when the source record is not the project root.
    public onGetRootTaskId() {
        if (this._sourceRecord.etn !== this._projectRecord.etn) return this._sourceRecord.getRecordId();
        return undefined;
    }

    public onIsTaskDeletingEnabled() {
        return !this._configService.getProperty('isTemplateView');
    }
}
```
