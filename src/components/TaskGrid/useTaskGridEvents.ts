...
export const useTaskGridEvents = (props: ITaskGridProps, datasetControl: ITaskGridDatasetControl, taskDataProvider: ITaskDataProvider) => {
    ...
    const { queryEvents } = datasetControl.getSavedQueryDataProvider();
    ...
    useEventEmitter<ISavedQueryDataProviderEvents>(queryEvents, 'onBeforeUserQueryCreated', (queryName: string) => props.onBeforeUserQueryCreated?.(queryName));
    useEventEmitter<ISavedQueryDataProviderEvents>(queryEvents, 'onAfterUserQueryCreated', (result: string | null) => props.onUserQueryCreated?.(result));
    useEventEmitter<ISavedQueryDataProviderEvents>(queryEvents, 'onBeforeUserQueryUpdated', (queryId: string) => props.onBeforeUserQueryUpdated?.(queryId));
    useEventEmitter<ISavedQueryDataProviderEvents>(queryEvents, 'onAfterUserQueryUpdated', (result: string | null) => props.onUserQueryUpdated?.(result));
    useEventEmitter<ISavedQueryDataProviderEvents>(queryEvents, 'onBeforeUserQueriesDeleted', (queryIds: string[]) => props.onBeforeUserQueriesDeleted?.(queryIds));
    useEventEmitter<ISavedQueryDataProviderEvents>(queryEvents, 'onAfterUserQueriesDeleted', (result: IDeletedUserQueriesResult) => props.onUserQueriesDeleted?.(result));
    useEventEmitter<ISavedQueryDataProviderEvents>(queryEvents, 'onError', (error: any, message: string) => props.onError?.(error, message));
