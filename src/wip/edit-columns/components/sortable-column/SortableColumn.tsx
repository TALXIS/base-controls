...
import { ICommandBarItemProps, useThemeGenerator } from "@talxis/react-components";
import { useEditColumns, useEditColumnsComponents, useEditColumnsLabels } from "../../context";

export const SortableColumn = (props: { column: IColumn }) => {
    const { column } = props;
    ...
    return (
        <ThemeProvider theme={sortableItemTheme} className={styles.sortableItem} ref={setNodeRef} style={style} {...attributes} {...listeners}>
            ...
            <components.SelectedColumnCommandBar
                styles={{
                    root: styles.commandBar,
                }}
                farItems={getCommandBarItems()}
            />
        </ThemeProvider>
    );
}